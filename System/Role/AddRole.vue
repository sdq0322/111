  ...
  export default {
    ...
    methods:{
        ...
        next(){
            ...
            //进入下一步之前 应该加一些判断【用户是否勾选了权限信息】
            const treeComponenList =  this.$refs.tree
            //每次点击下一步之前先清空perms中原有数据
            this.roleForm.perms = []
            treeComponenList.forEach(tree=>{
                this.roleForm.perms.push(tree.getCheckedKeys())
            })
            //把二维数组展开为一维数组
            //console.log(this.roleForm.perms.flat())
            if(this.roleForm.perms.flat().length<=0){
                this.$message.warning('请选择权限信息')
                return
            }
            const diabledTreeList =  this.$refs.diabledTree
            diabledTreeList.forEach((tree,index)=>{
